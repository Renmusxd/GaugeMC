...
fn main() -> Result<(), CudaError> {
    ...
    let ks = if num_replicas > 1 {
        (0..num_replicas)
            .map(|i| (khigh - klow) * (i as f32 / (num_replicas - 1) as f32) + klow)
            .collect()
    } else {
        vec![(khigh + klow) / 2.0]
    };
    ...
    output
        ...
        .try_for_each(|mut arr| -> Result<(), CudaError> {
            ...
            let total_per_replica = edge_violations
                .axis_iter(Axis(0))
                .map(|x| x.into_iter().map(|x| x.abs()).sum::<i32>());
    ...
    npz.add_array("perimeters", &output)
